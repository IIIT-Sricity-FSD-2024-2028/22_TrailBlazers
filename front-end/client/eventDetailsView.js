class EventDetailsView {
    static render(eventId) {
        ...
        const isClient = user && (user.role === 'client' || user.role === 'eventmanager' || user.role === 'superuser');
                ...
                <div style="display:flex; flex-direction:column; gap: 30px;">
                    <div class="card">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 20px;">
                            ...
                            ${isClient
                              ? `<a href="#/edit-event?id=${id}" class="btn btn-secondary btn-sm">✏️ Edit Event</a>`
                              : `<button class="btn btn-primary btn-sm rsvp-btn" data-id="${id}">✅ RSVP Now</button>`}
                        </div>
