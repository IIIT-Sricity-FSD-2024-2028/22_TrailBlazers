            ...
            <div id="sessionModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:1000; align-items:center; justify-content:center;">
                <div class="card" style="width: 500px; padding: 30px;">
                    ...
                    <div style="display:flex; gap:10px; justify-content:flex-end;">
                        <button id="btnCancelSession" type="button" class="btn btn-secondary">Cancel</button>
                        <button id="btnSaveSession" type="button" class="btn btn-primary">Save Session</button>
                    </div>
                </div>
            </div>
        `;
    ...
    static init(eventId = null) {
        ...
        document.getElementById('btnCreateEvent').addEventListener('click', async () => {
            ...
            const eventData = {
                ...
                // Client info (filled when a client creates the event)
                clientId:    isClient ? currentUser.id    : null,
                clientEmail: isClient ? currentUser.email : null,
                clientName:  isClient ? currentUser.name  : null,
                // Manager info (assigned later on approval; use current user if they ARE a manager)
                managerId:   isClient ? null : currentUser.id,
                managerName: isClient ? null : currentUser.name,
                domain: currentUser.domain || 'General',
                status: 'pending'
            };

            const capacityRaw = document.getElementById('eventCapacity').value.trim();
            const capacityVal = parseInt(capacityRaw, 10);

            if (!eventData.title || !eventData.venue || !eventData.date || !capacityRaw || isNaN(capacityVal)) {
                alert('Please fill out all required fields (Name, Venue, Date, Capacity).');
                return;
            }

            eventData.capacity = capacityVal;
            ...
            if (eventData.capacity <= 0) {
                ...
                if (this.currentId) {
                    ...
                    await fetch(`http://localhost:3000/events/${this.currentId}`, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json', 'role': currentUser.role || 'eventmanager' },
                    ...
                    // ── Create new event ─────────────────────────────────
                    const roleHeader = (currentUser && currentUser.role) ? currentUser.role : 'eventmanager';
                    ...
                    const newEventFromBackend = await res.json();
                    eventData.id = newEventFromBackend.id;
                    DataStore.addEvent({ ...eventData, ...newEventFromBackend });

                    // ── Also submit pending request (client flow) ────────
                    if (isClient) {
                        await fetch('http://localhost:3000/pending-requests', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'role': 'client' },
                            body: JSON.stringify({
                                eventId:     newEventFromBackend.id,
                                name:        eventData.title,
                                location:    eventData.venue || eventData.location || '',
                                expected:    eventData.capacity,
                        ...
                        // ── Notify event managers about the new request ──────
                        try {
                            await fetch('http://localhost:3000/notifications', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json', 'role': 'client' },
                                body: JSON.stringify({
                                    title:    '📋 New Event Request: ' + eventData.title,
                                    desc:     `Client ${currentUser.name} (${currentUser.email}) submitted a new event request for “${eventData.title}” on ${eventData.date}.`,
                                    priority: 'medium',
                                    reporter: currentUser.name || 'Client'
                                })
                            });
                        } catch(e) { console.warn('Notification failed:', e); }

                        alert('Event created and submitted for manager approval!');
                    } else {
                        alert('Event created successfully!');
                    }
                }
                // Force cache refresh so My Events reloads from backend
                window._datastoreCacheLoaded = false;
                window.location.hash = '#/my-events';
            } catch (err) {
                alert('Failed to save to backend: ' + err.message);
            }
        });
        ...
        document.getElementById('btnSaveDraft').addEventListener('click', async () => {
            const eventData = {
                ...
                status: 'pending' // Note: Using 'pending' to match data modeling
            };
            ...
            try {
                ...
                alert('Failed to save draft to backend: ' + err.message);
            }
        });
