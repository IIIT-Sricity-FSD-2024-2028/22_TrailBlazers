class EventsView {
  ...
  static renderMyEvents() {
    ...
    // Filter events based on logged-in user's role
    if (user.role === 'client') {
      // Clients see only events they own
      events = events.filter((e) => e.clientId === user.id || e.clientEmail === user.email);
    } else if (user.role === 'eventmanager') {
      // Event managers see only events assigned to them
      events = events.filter((e) => e.managerId === user.id);
    } else if (user.role === 'superuser') {
      // Superuser sees all events (no filter)
    }
  ...
  static renderEventCard(ev, isPublicView) {
    ...
    if (isPublicView) {
      ...
      const user = Auth.getCurrentUser();
      const isClientOwner = user && user.role === 'client' &&
        (ev.clientId === user.id || ev.clientEmail === user.email);

      if (isPending && isClientOwner) {
        // Client's own pending event — show edit + approval status note
        actionsHtml = `
          <a href="#/edit-event?id=${ev.id}" class="btn btn-secondary btn-block">✏️ Edit Event</a>
          <div style="margin-top:6px; padding:8px 10px; background:#fff7ed; border:1px solid #fed7aa; border-radius:8px; font-size:11px; color:#c2410c; text-align:center; line-height:1.4;">
            ⏳ Awaiting manager<br>approval
          </div>`;
      } else if (canManage) {
        // Manager/superuser or approved event — full controls
        actionsHtml = `
      ...
      document.addEventListener("click", async (e) => {
        ...
        if (deleteBtn) {
          ...
          if (confirm(`Are you sure you want to delete "${name}"?`)) {
            ...
            if (main) main.innerHTML = EventsView.renderMyEvents();
