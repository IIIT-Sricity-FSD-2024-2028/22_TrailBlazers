class EngagementView {
    ...
    static renderTab(tabId) {
        ...
        if (tabId === 'poll') {
            ...
            content.innerHTML = '<p style="padding:20px;color:var(--text-muted);">Loading questions…</p>';
            this.getQAHtml().then(html => {
                content.innerHTML = html;
                this.attachQAListeners();
                // Refresh button
                document.getElementById('refreshQnaBtn')?.addEventListener('click', () => {
                    content.innerHTML = '<p style="padding:20px;color:var(--text-muted);">Loading questions…</p>';
                    this.getQAHtml().then(h => { content.innerHTML = h; this.attachQAListeners(); });
                });
            });
    ...
    static async getQAHtml() {
        // Get current event context
        let eventId = 'e1';
        try {
            const user = (typeof App !== 'undefined' && App.user) || (typeof Auth !== 'undefined' && Auth.getCurrentUser()) || {};
            eventId = user.eventId || user.assignedEventId ||
                      sessionStorage.getItem('active_event_id') || 'e1';
        } catch(_) {}
        ...
        try {
            ...
            console.warn('[Q&A] Could not load questions from backend:', err.message);
        }
        ...
        if (!Array.isArray(questions) || !questions.length) {
            return `
                <div class="card" style="padding: 30px;">
                    ...
                    <p style="color:var(--text-muted); font-size:14px;">No questions submitted yet for this event.</p>
                </div>
            `;
        }

        const listHtml = questions.map(q => `
            <div class="qa-item" data-qna-id="${q.id}" style="padding:15px; border:1px solid var(--border-color); border-radius:8px; background:white;">
                <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                    <div>
                        <div style="font-weight:600; margin-bottom:5px;">${q.question.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>
                        <div style="color:var(--text-muted); font-size:12px;">Asked by ${q.askedBy} &bull; <span class="qa-status">${q.status === 'answered' ? '<span style="color:var(--success);font-weight:600;">Answered ✔️</span>' : 'Pending'}</span></div>
                    </div>
                    <div style="display:flex; gap:10px;" class="qa-actions" ${q.status === 'answered' ? 'style="display:none;"' : ''}>
                        ${q.status !== 'answered' ? '<button class="btn btn-primary btn-sm qa-attend">Answer</button><button class="btn btn-danger btn-sm qa-dismiss">Dismiss</button>' : ''}
                    </div>
                </div>
                
                ${q.answer ? `<div class="qa-reply-display" style="margin-top:10px; padding:10px; background:var(--sidebar-bg); border-radius:6px; font-size:13px;"><strong>Your Reply:</strong> <span class="qa-reply-content">${q.answer.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</span></div>` : `
        ...
        return `
            <div class="card" style="padding: 30px;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                    <h3>Attendee Questions Platform</h3>
                    <button class="btn btn-secondary btn-sm" id="refreshQnaBtn" style="font-size:12px;">🔄 Refresh</button>
                </div>
        ...
        document.querySelectorAll('.qa-item').forEach(item => {
            ...
            if (sendBtn) {
                sendBtn.addEventListener('click', async () => {
                    ...
                    // Get current client user for answeredBy
                    let clientUser = {};
                    try { clientUser = (typeof App !== 'undefined' && App.user) || (typeof Auth !== 'undefined' && Auth.getCurrentUser()) || {}; } catch(_) {}

                    if (qnaId) {
                        try {
                            const res = await fetch(`${API_BASE}/qna/${qnaId}/answer`, {
                                method: 'PATCH',
                                headers: { 'Content-Type': 'application/json', role: 'client' },
                                body: JSON.stringify({
                                    answer: text,
                                    answeredBy: clientUser.id || clientUser.email || 'client',
                                }),
                            });
                            if (!res.ok) {
                                const err = await res.json().catch(() => ({}));
                                alert('Could not save answer: ' + (err.message || res.status));
                                return;
                            }
                            console.log('[Q&A] ✅ Answer saved to backend for', qnaId);
                        } catch (err) {
                            console.warn('[Q&A] Answer not saved to backend:', err.message);
                        }
                    }
                    ...
                    if (statusBox) statusBox.innerHTML = '<span style="color:var(--success); font-weight:600;">Answered ✔️</span>';
                    item.style.background = 'var(--sidebar-bg)';
                    const actionsEl = item.querySelector('.qa-actions');
...
class ReportsView {
    static render() {
        ...
        const isOsc = user.role === 'osc' || user.role === 'superuser';

        return `
            <div class="page-header" style="margin-bottom: 20px;">
                <div class="page-title">
                    <h2>Reports${isOsc ? ' & Check-In Station' : ''}</h2>
                    <p>${isOsc ? 'Check in attendees and download analytics' : 'Download analytics and compare past events'}</p>
                </div>
            </div>

            ${isOsc ? `
            <!-- ── OSC CHECK-IN STATION ──────────────────────────────── -->
            <div class="card" style="margin-bottom: 30px; border-left: 4px solid var(--primary);">
                <h3 style="margin:0 0 18px 0; font-size:16px; display:flex; align-items:center; gap:8px;">
                    <span style="font-size:20px;">✅</span> Check-In Station
                </h3>

                <div style="display:grid; grid-template-columns: repeat(3, 1fr) auto; gap:12px; align-items:end;">
                    <div class="form-group" style="margin:0;">
                        <label style="font-size:12px; font-weight:600; color:var(--text-muted);">Attendee Name</label>
                        <input type="text" id="ci-name" placeholder="e.g. Rahul Sharma" style="width:100%; padding:10px 14px; border-radius:8px; border:1px solid var(--border-color); background:var(--bg-color); color:var(--text-main); font-size:14px;">
                    </div>
                    <div class="form-group" style="margin:0;">
                        <label style="font-size:12px; font-weight:600; color:var(--text-muted);">Attendee Email</label>
                        <input type="email" id="ci-email" placeholder="e.g. rahul@gmail.com" style="width:100%; padding:10px 14px; border-radius:8px; border:1px solid var(--border-color); background:var(--bg-color); color:var(--text-main); font-size:14px;">
                    </div>
                    <div class="form-group" style="margin:0;">
                        <label style="font-size:12px; font-weight:600; color:var(--text-muted);">Event ID</label>
                        <input type="text" id="ci-eventId" placeholder="e.g. e1" value="${user.assignedEventId || ''}" style="width:100%; padding:10px 14px; border-radius:8px; border:1px solid var(--border-color); background:var(--bg-color); color:var(--text-main); font-size:14px;">
                    </div>
                    <button id="checkInBtn" class="btn btn-primary" style="padding:10px 22px; white-space:nowrap;">
                        ✅ Check In
                    </button>
                </div>
                ...
                <div style="margin-top:24px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                        <h4 style="margin:0; font-size:14px; font-weight:600;">Today's Check-Ins</h4>
                        <button id="refreshCheckIns" class="btn btn-secondary" style="padding:6px 14px; font-size:12px;">🔄 Refresh</button>
                    </div>
                    <div id="ci-list" style="max-height:260px; overflow-y:auto; border-radius:8px; border:1px solid var(--border-color);">
    ...
    // ── Load and render today's check-ins from backend ────────────────────────
    static async _loadCheckIns(eventId) {
        const list = document.getElementById('ci-list');
        if (!list) return;
        try {
            const url = `http://localhost:3000/check-ins${eventId ? `?eventId=${eventId}` : ''}`;
            const res = await fetch(url, { headers: { role: 'osc' } });
            const checkins = await res.json();
            if (!checkins.length) {
                list.innerHTML = '<div style="padding:20px; text-align:center; color:var(--text-muted); font-size:13px;">No check-ins yet.</div>';
                return;
            }
            list.innerHTML = checkins.map(c => `
                <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 16px; border-bottom:1px solid var(--border-color);">
                    <div>
                        <div style="font-weight:600; font-size:13px; color:var(--text-main);">${c.attendeeName || c.attendeeEmail}</div>
                        <div style="font-size:11px; color:var(--text-muted);">${c.attendeeEmail} &nbsp;·&nbsp; Event: ${c.eventId}</div>
                    </div>
                    <div style="text-align:right;">
                        <span style="background:${c.status === 'checked-in' ? '#10b98120' : '#f9731620'}; color:${c.status === 'checked-in' ? '#10b981' : '#f97316'}; padding:3px 10px; border-radius:10px; font-size:11px; font-weight:700;">
                            ${c.status === 'checked-in' ? '✅ Checked In' : '⏳ Pending'}
                        </span>
                        <div style="font-size:10px; color:var(--text-muted); margin-top:2px;">${c.checkInTime ? new Date(c.checkInTime).toLocaleTimeString() : ''}</div>
                    </div>
                </div>`).join('');
        } catch(e) {
            list.innerHTML = '<div style="padding:20px; text-align:center; color:#ef4444; font-size:13px;">Failed to load check-ins. Is the backend running?</div>';
        }
    }
    ...
    static async init() {
        ...
        // ── Check-In Station ──────────────────────────────────────────────────
        if (isOsc) {
            const eventIdInput = document.getElementById('ci-eventId');
            await this._loadCheckIns(eventIdInput?.value?.trim());

            document.getElementById('refreshCheckIns')?.addEventListener('click', () => {
                this._loadCheckIns(document.getElementById('ci-eventId')?.value?.trim());
            });

            document.getElementById('checkInBtn')?.addEventListener('click', async () => {
                const name    = document.getElementById('ci-name')?.value?.trim();
                const email   = document.getElementById('ci-email')?.value?.trim();
                const eventId = document.getElementById('ci-eventId')?.value?.trim();
                const statusEl = document.getElementById('ci-status');

                if (!name || !email || !eventId) {
                    statusEl.innerHTML = '<span style="color:#ef4444;">⚠️ Please fill in Name, Email and Event ID.</span>';
                    return;
                }
                ...
                try {
                    ...
                    // Single atomic call: create + mark as checked-in in one request
                    const createRes = await fetch('http://localhost:3000/check-ins', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', role: 'osc' },
                        body: JSON.stringify({
                            eventId,
                            attendeeId:    email.split('@')[0],
                            attendeeEmail: email,
                            attendeeName:  name,
                            ticketCode,
                            status:        'checked-in',
                    ...
                    if (!createRes.ok) {
                        ...
                        throw new Error(`POST /check-ins failed: ${errText}`);
                    }
                    const created = await createRes.json();
                    if (!created || !created.id) throw new Error('Backend returned no ID.');

                    statusEl.innerHTML = `<span style="color:#10b981;">✅ <strong>${name}</strong> checked in! Ticket: ${ticketCode} · ID: ${created.id}</span>`;
                    toast(`✅ ${name} checked in!`, '#10b981');

                    // Clear inputs & refresh list
                    document.getElementById('ci-name').value  = '';
                    document.getElementById('ci-email').value = '';
                    await this._loadCheckIns(eventId);

                } catch(e) {
                    statusEl.innerHTML = `<span style="color:#ef4444;">❌ ${e.message}</span>`;
                    toast('Check-in failed!', '#ef4444');
                } finally {
                    btn.disabled = false;
...
class SettingsView {
    static render() {
        const user = App.user || Auth.getCurrentUser() || {};
        const roleNames = {
            'superuser': 'Super User', 'client': 'Client',
            'eventmanager': 'Event Manager', 'attendee': 'Attendee', 'osc': 'On-Site Coordinator'
        };
        return `
            <div class="page-header" style="margin-bottom: 20px;">
                <div class="page-title">
                    <h2>Settings</h2>
                    <p>Your account profile and preferences</p>
                </div>
            </div>

            <div class="tabs" style="gap: 20px;">
                <div class="tab active"><span style="color:var(--primary); font-weight:600;">My Profile</span></div>
            </div>

            <div class="card">
                <h3 style="margin-bottom: 20px; display:flex; align-items:center; gap: 10px;">
                    <span style="color:var(--primary);">👤</span> My Profile
                </h3>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label>Role</label>
                        <input type="text" readonly value="${roleNames[user.role] || user.role || ''}" style="background:var(--sidebar-bg); cursor:not-allowed;">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label>User ID</label>
                        <input type="text" readonly value="${user.id || ''}" style="background:var(--sidebar-bg); cursor:not-allowed;">
                    </div>
                    <div class="form-group">
                        <label>Domain / Specialization</label>
                        <input type="text" id="profile-domain" value="${user.domain || user.specialization || ''}">
                    </div>
                </div>
        ...
        if (saveBtn) {
            saveBtn.addEventListener('click', async () => {
                ...
                const updates = {
                    ...
                    domain: document.getElementById('profile-domain')?.value.trim(),
                };
                try {
                    ...
                    localStorage.setItem('wevents_user', JSON.stringify(updatedUser));
                    // Refresh topbar name/email
                    document.getElementById('userName') && (document.getElementById('userName').textContent = updatedUser.name);
                    document.getElementById('userEmail') && (document.getElementById('userEmail').textContent = updatedUser.email);
                    document.getElementById('userInitial') && (document.getElementById('userInitial').textContent = updatedUser.name.charAt(0).toUpperCase());
                    document.getElementById('ddUserName') && (document.getElementById('ddUserName').textContent = updatedUser.name);
                    document.getElementById('ddUserEmail') && (document.getElementById('ddUserEmail').textContent = updatedUser.email);
                    toast('Profile saved successfully!', '#22c55e');
                } catch(e) {
                    toast('Failed to save profile: ' + e.message, '#ef4444');
                }
            });
        }
    }
...
class NotificationsView {
        ...
        return `
            ...
            <div class="dashboard-grid" style="grid-template-columns: repeat(4, 1fr); margin-bottom: 30px;">
                <div class="card stat-card" style="align-items:flex-start;">
                    <div class="stat-header"><h3 style="font-size:13px; color:var(--text-main); font-weight:600;">Total</h3></div>
                    <div class="stat-value" style="font-size:24px; color:var(--text-main);" id="notif-total">–</div>
                </div>
                <div class="card stat-card" style="align-items:flex-start;">
                    <div class="stat-header"><h3 style="font-size:13px; color:var(--text-main); font-weight:600;">Unread</h3></div>
                    <div class="stat-value" style="font-size:24px; color:var(--primary);" id="notif-unread">–</div>
                </div>
                <div class="card stat-card" style="align-items:flex-start;">
                    <div class="stat-header"><h3 style="font-size:13px; color:var(--text-main); font-weight:600;">Critical</h3></div>
                    <div class="stat-value" style="font-size:24px; color:#ef4444;" id="notif-critical">–</div>
                </div>
                <div class="card stat-card" style="align-items:flex-start;">
                    <div class="stat-header"><h3 style="font-size:13px; color:var(--text-main); font-weight:600;">Resolved</h3></div>
                    <div class="stat-value" style="font-size:24px; color:var(--success);" id="notif-resolved">–</div>
                </div>
            </div>
    ...
    static _priorityColor(p) {
        return p === 'critical' ? '#ef4444' : p === 'high' ? '#f97316' : p === 'medium' ? '#3b82f6' : '#10b981';
    }

    static _renderCard(n) {
        const color  = this._priorityColor(n.priority);
        const isRead = n.isRead;
        const timeAgo = n.createdAt ? new Date(n.createdAt).toLocaleString() : n.time || '';
        return `
        <div id="notif-card-${n.id}" style="padding:18px 20px; border-bottom:1px solid var(--border-color); display:flex; gap:15px; background:${isRead ? 'transparent' : 'rgba(59,130,246,0.04)'}; transition:background 0.3s;">
            <div style="min-width:40px; height:40px; border-radius:50%; background:${color}22; color:${color}; display:flex; align-items:center; justify-content:center; font-size:18px;">${n.priority === 'critical' ? '🚨' : n.priority === 'high' ? '🔴' : n.priority === 'medium' ? '🟡' : '🟢'}</div>
            <div style="flex:1;">
                <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
                    <div style="display:flex; align-items:center; gap:8px;">
                        <h4 style="margin:0; font-size:14px; color:var(--text-main); font-weight:600;">${n.title}</h4>
                        ${!isRead ? '<span class="badge" style="background:var(--primary); color:white; border:none; font-size:10px;">New</span>' : ''}
                        <span style="background:${color}22; color:${color}; padding:2px 8px; border-radius:10px; font-size:10px; font-weight:700; text-transform:uppercase;">${n.priority}</span>
                    </div>
                    <span style="font-size:11px; color:var(--text-muted);">${timeAgo}</span>
                </div>
                <p style="margin:0 0 8px; font-size:13px; color:var(--text-muted);">${n.desc || ''}${n.reporter ? ' — <em>' + n.reporter + '</em>' : ''}</p>
                <div style="display:flex; gap:8px;">
                    ${!isRead ? `<button class="btn btn-secondary btn-sm" onclick="NotificationsView._markRead('${n.id}')">Mark as Read</button>` : '<span style="font-size:12px; color:var(--text-muted);">✓ Read</span>'}
                    ${n.status !== 'resolved' ? `<button class="btn btn-sm" style="background:#10b981; color:white; border:none; padding:6px 12px; border-radius:6px; font-size:12px; cursor:pointer;" onclick="NotificationsView._resolve('${n.id}')">✔ Resolve</button>` : '<span style="font-size:12px; color:var(--success);">✔ Resolved</span>'}
                </div>
            </div>
        </div>`;
    }

    static async _markRead(id) {
        try {
            ...
            if (card) {
                ...
                if (btn && btn.textContent === 'Mark as Read') {
                    btn.outerHTML = '<span style="font-size:12px; color:var(--text-muted);">✓ Read</span>';
                }
            }
            this._updateBadge();
    ...
    static async _resolve(id) {
        try {
            await fetch(`http://localhost:3000/notifications/${id}/resolve`, {
                method: 'PATCH',
                headers: { 'role': 'eventmanager' }
            });
            const card = document.getElementById(`notif-card-${id}`);
            if (card) {
                const resolveBtn = card.querySelectorAll('button')[1] || card.querySelector('button[style*="10b981"]');
                if (resolveBtn) resolveBtn.outerHTML = '<span style="font-size:12px; color:var(--success);">✔ Resolved</span>';
            }
        } catch(e) { console.warn('resolve failed', e); }
    }

    static async _updateBadge() {
        try {
            const res  = await fetch('http://localhost:3000/notifications/unread-count', { headers: { role: 'eventmanager' } });
    ...
    static async init() {
        try {
            const user = Auth.getCurrentUser();
            const role = (user && user.role) || 'eventmanager';
            const res  = await fetch('http://localhost:3000/notifications', { headers: { role } });
            ...
            document.getElementById('notif-total')    && (document.getElementById('notif-total').textContent    = total);
            document.getElementById('notif-unread')   && (document.getElementById('notif-unread').textContent   = unread);
            document.getElementById('notif-critical') && (document.getElementById('notif-critical').textContent = critical);
            document.getElementById('notif-resolved') && (document.getElementById('notif-resolved').textContent = resolved);
            ...
            if (list) {
                if (notifications.length === 0) {
                    list.innerHTML = '<p style="padding:30px 20px; color:var(--text-muted);">No notifications yet.</p>';
                } else {
                    list.innerHTML = notifications.map(n => this._renderCard(n)).join('');
                }
            }
            ...
            if (markAllBtn) {
                markAllBtn.addEventListener('click', async () => {
                    ...
                    await Promise.all(unreadItems.map(n => this._markRead(n.id)));
                    this._toast('All notifications marked as read', '#22c55e');
                });
            }

        } catch(e) {
            const list = document.getElementById('notif-list');
            if (list) list.innerHTML = '<p style="padding:30px 20px; color:#ef4444;">Failed to load notifications. Ensure the backend is running.</p>';
            console.error(e);
        }
    }
}
