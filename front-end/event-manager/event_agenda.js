...
function renderSessions() {
  ...
  c.innerHTML = filtered.map(s => {
    ...
    const actions = s.status === 'live'
      ? `<button class="btn btn-pause" onclick="togglePause(${i})">
           ${isPaused ? '<i class="fa-solid fa-play"></i>Resume' : '<i class="fa-solid fa-pause"></i>Pause'}
         </button>
         <button class="btn btn-end" onclick="endSession(${i})"><i class="fa-regular fa-square"></i>End Session</button>`
      : s.status === 'upcoming'
      ? `<button class="btn btn-edit" onclick="openModal(${i})">Adjust Time</button>`
      : '';
...
function togglePause(i) {
  sessions[i].isPaused = !sessions[i].isPaused;
  render();
  toast(sessions[i].isPaused ? 'Session Paused' : 'Session Resumed');
}
