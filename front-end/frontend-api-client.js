...
const ApiClient = {
  ...
  rsvps: {
    ...
    getMy: (email) => apiFetch('GET', `/rsvps/my?email=${encodeURIComponent(email)}`, 'attendee'),
    getOne: (id, role) => apiFetch('GET', `/rsvps/${id}`, role),
    create: (data) => apiFetch('POST', '/rsvps', 'attendee', data),
    ...
    cancel: (id, role = 'attendee') => apiFetch('DELETE', `/rsvps/${id}`, role),
  },
  ...
  pendingRequests: {
    ...
    create: (data, role = 'attendee') => apiFetch('POST', '/pending-requests', role, data),
