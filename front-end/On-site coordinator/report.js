...
reportBtn.addEventListener('click', () => {
    ...
    // Notify team for local resolution
    if (window.addTeamNotification) {
        // In a real app we'd get the actual user. Hardcoding Alex Thompson for now.
        window.addTeamNotification(title, desc, priority, 'Alex Thompson');
    }
...
// ── Confirm Escalation ────────────────────────────────────────────────────────
confirmEscalate.addEventListener('click', () => {
    // Write notification to localStorage for Event Manager
    ...
    let notifications = JSON.parse(localStorage.getItem('wevents_em_notifications')) || [];
    notifications.unshift({
        ...
        read: false,
        source: 'On-Site Coordinator'
    });
    localStorage.setItem('wevents_em_notifications', JSON.stringify(notifications));
