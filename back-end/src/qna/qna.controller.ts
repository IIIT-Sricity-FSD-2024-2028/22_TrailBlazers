...
export class QnaController {
  ...
  // ─── GET ALL Q&A ─────────────────────────────────────────────────────────────
  @Get()
  @Roles(Role.CLIENT, Role.EVENTMANAGER, Role.OSC, Role.ATTENDEE)
  @ApiOperation({ summary: 'Get all Q&A items', description: 'Returns all questions, optionally filtered by eventId or status. Accessible by client, eventmanager, osc, attendee, and superuser.' })
  ...
  @Get('stats/:eventId')
  @Roles(Role.CLIENT, Role.EVENTMANAGER, Role.OSC)
  ...
  // ─── UPDATE QUESTION TEXT (ATTENDEE EDIT) ────────────────────────────────────
  @Patch(':id')
  @Roles(Role.ATTENDEE, Role.CLIENT, Role.EVENTMANAGER)
  @ApiOperation({ summary: 'Update a question', description: 'Attendee can edit their own question text; client/eventmanager can also update.' })
  @ApiHeader({ name: 'role', description: 'superuser | attendee | client | eventmanager', required: true })
  @ApiParam({ name: 'id', description: 'Q&A item ID' })
  @ApiResponse({ status: 200, description: 'Q&A item updated' })
  ...
  @ApiOperation({ summary: 'Delete a Q&A item', description: 'Client or eventmanager permanently removes a Q&A entry.' })
  @ApiHeader({ name: 'role', description: 'Must be: superuser | client | eventmanager', required: true, example: 'client' })
  @ApiParam({ name: 'id', description: 'Q&A item ID' })
  @ApiResponse({ status: 200, description: 'Q&A item deleted' })
