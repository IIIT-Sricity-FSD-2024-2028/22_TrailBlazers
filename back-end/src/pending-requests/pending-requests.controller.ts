...
export class PendingRequestsController {
  ...
  @ApiOperation({ summary: 'Get requests by client email' })
  @ApiHeader({ name: 'role', description: 'Must be: client | eventmanager | superuser', required: true })
  @ApiQuery({ name: 'email', required: true, description: 'Client email address' })
  @ApiResponse({ status: 200, description: 'Requests for this client' })
  findByClient(@Query('email') email: string) {
    return this.service.findByClient(email);
  }
