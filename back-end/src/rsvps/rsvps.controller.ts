...
export class RsvpsController {
  ...
  @Post()
  @Roles(Role.ATTENDEE)
  @ApiOperation({ summary: 'Create RSVP (Register for an event)', description: 'Attendee or superuser registers for an event. Phone must be exactly 10 digits.' })
  @ApiHeader({ name: 'role', description: 'Must be: attendee | superuser', required: true, example: 'attendee' })
  ...
  @Patch(':id')
  @Roles(Role.EVENTMANAGER, Role.OSC)
  @ApiOperation({ summary: 'Update RSVP status', description: 'Update RSVP status or details. Accessible by superuser, eventmanager, and OSC only.' })
  @ApiHeader({ name: 'role', description: 'Must be: superuser | eventmanager | osc', required: true })
  @ApiParam({ name: 'id', description: 'RSVP ID' })
  @ApiResponse({ status: 200, description: 'RSVP updated' })
  @ApiResponse({ status: 403, description: 'Only superuser, eventmanager or osc can update RSVP status' })
  ...
  @ApiOperation({ summary: 'Cancel / delete RSVP', description: 'Permanently delete an RSVP. Accessible by superuser and eventmanager only.' })
  @ApiHeader({ name: 'role', description: 'Must be: superuser | eventmanager', required: true })
  @ApiParam({ name: 'id', description: 'RSVP ID' })
  @ApiResponse({ status: 200, description: 'RSVP deleted successfully' })
  @ApiResponse({ status: 403, description: 'Only superuser or eventmanager can delete RSVPs' })
  @ApiResponse({ status: 404, description: 'RSVP not found' })
