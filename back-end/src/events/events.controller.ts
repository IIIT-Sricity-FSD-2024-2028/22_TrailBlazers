...
export class EventsController {
  ...
  @ApiQuery({ name: 'clientId', required: false, description: 'Filter by client user ID (e.g. u2). Returns events created by that client.' })
  ...
  findAll(
    ...
    return this.eventsService.findAll(status, managerId, clientId);
  }
  ...
  @ApiHeader({ name: 'role', description: 'User role (superuser | eventmanager | client)', required: true })
  ...
  @ApiOperation({ summary: 'Create a new event', description: 'Accessible by superuser, eventmanager and client.' })
  ...
  @ApiResponse({ status: 403, description: 'Only superuser, eventmanager or client can create events' })
  ...
  @Put(':id')
  @Roles(Role.SUPERUSER, Role.EVENTMANAGER, Role.CLIENT)
  @ApiOperation({ summary: 'Update event (full replace)', description: 'Full update of an event. Superuser, eventmanager and client only.' })
  @ApiHeader({ name: 'role', description: 'Must be: superuser | eventmanager | client', required: true, example: 'eventmanager' })
  ...
  @Patch(':id')
  @Roles(Role.SUPERUSER, Role.EVENTMANAGER, Role.CLIENT)
  @ApiOperation({ summary: 'Partially update event', description: 'Update specific fields. Superuser, eventmanager and client only. Use for status changes (e.g. pending → live).' })
  @ApiHeader({ name: 'role', description: 'Must be: superuser | eventmanager | client', required: true, example: 'eventmanager' })
  @ApiParam({ name: 'id', description: 'Event ID' })
  ...
  @ApiOperation({ summary: 'Delete event', description: 'Permanently remove an event. Superuser, eventmanager or client.' })
  @ApiHeader({ name: 'role', description: 'Must be: superuser | eventmanager | client', required: true, example: 'eventmanager' })
  @ApiParam({ name: 'id', description: 'Event ID' })
